...
Choose a location for your POLIS data folder, this can be on your local computer or, if you have access to Microsoft's Azure environment, in Azure storage.

```
library(tidypolis)
init_tidypolis <- function(
    polis_folder = "path/to/POLIS", #replace with your file path
    edav = F, #change to T if using Azure storage
    api_debug = F #set to T to log individual API calls
    )
```

This function will create a POLIS folder if one doesn't exist or connect to an already existing POLIS folder. If creating folder you will be required to supply a valid POLIS API key. If the folder already exists your API key will be validated with POLIS. (Note: if POLIS is offline for any reason, ie maintenance, you will be unable to validate your API key)

In creating your POLIS folder your API key will be stored in a 'creds.rds' 
